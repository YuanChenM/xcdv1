...
import com.msk.core.exception.DataNotFoundException;
...
public class BaseLogic {
    ...
    public <T extends BaseEntity> T load(BaseParam param) {
        ...
        if (entity == null) {
            throw new DataNotFoundException("没有找到你想要的数据");
        }
        return entity;
    }
    ...
    public <T extends BaseEntity> PageResult<T> findPage(BaseParam param, Class<T> clazz) {
        ...
        if (pageResult.getRecordsTotal() != IntDef.INT_ZERO) {
    ...
    public <T extends BaseEntity> List<T> findPageList(RsPageParam param, RsPageResult pageResult) {
        ...
        if (pageResult.getTotalCount() != IntDef.INT_ZERO) {
    ...
    public <T extends BaseEntity> List<T> findAll(){
        ...
        return this.baseDao.selectList(this.getSqlId(CommSqlId.FIND_ALL), param);
    }
