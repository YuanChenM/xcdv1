...
public class PoiUtils {
	...
	private static boolean checkExcelRow(Sheet sheet, int row) {
		...
		if (sheet instanceof HSSFSheet) {
			if (row < IntDef.INT_ZERO || row > NumberConst.ExcelMaxRowCount.EXCEL_2003_MAX_ROW_COUNT) {
				return Boolean.FALSE;
			}
		} else {// Excel 2003以外的
			if (row < IntDef.INT_ZERO || row > NumberConst.ExcelMaxRowCount.EXCEL_2007_MAX_ROW_COUNT) {
				return Boolean.FALSE;
			}
		}
		return Boolean.TRUE;
	...
	private static boolean checkExcelColumn(Sheet sheet, int row, int column) {
		...
		if (sheet instanceof HSSFSheet) {
			if (column < IntDef.INT_ZERO || column > NumberConst.ExcelMaxColumnCount.EXCEL_2003_MAX_COLUMN_COUNT) {
				return Boolean.FALSE;
			}
		} else {// Excel 2003以外的
			if (column < IntDef.INT_ZERO || column > NumberConst.ExcelMaxColumnCount.EXCEL_2007_MAX_COLUMN_COUNT) {
				return Boolean.FALSE;
			}
		}
		return Boolean.TRUE;
