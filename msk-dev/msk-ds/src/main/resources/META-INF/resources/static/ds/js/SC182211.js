...
var SC182211 = {
        ...
        $("#weightCode").change(function(){
            ...
            if(null!=$("#weightCode").val() && ''!=$("#weightCode").val()) {
                $("#main-content").postUrl(Main.contextPath + "/SC182211/querySlPropEp", formData, function (data) {
                    if (null != data && data.length >= 1) {
                        $(data).each(function (i, val) {
                            $("#prodEpId").append("<option value='" + val.slCodeDis + "'>" + val.epName + "</option>").find("option").eq(i + 1).attr("valuetwo", val.slCodeManufacture);;
                        });
                    }
                }, {refreshHtml: false});
    ...
    saveData:function(){
        $("#SC182211_SAVE").click(function(){
            if(''!=$("#slCode").val() &&''!=$("#lgcsCode").val()&&''!=$("#pdClassesCode").val() && ''!=$("#machiningCode").val() && ''!=$("#pdBreedCode").val() && ''!=$("#pdFeatureCode").val() && ''!=$("#groupCode").val() && ''!=$("#prodEpId").val() && ''!=$("#platform").val() && ''!=$("#brandId").val() && ''!=$("#priceDate").val() && ''!=$("#half").val() && ''!=$("#pageNumber").val() && ''!=$("#normsCode").val() && ''!=$("#slPdArtno").val() && ''!=$("#weightCode").val()){
                formData = getFormData($("#" + SC182211.formId));
                ...
                formData.slCodeManufacture=$("#prodEpId").find("option:selected").attr("valuetwo");
                ...
                formData.lgcsCode=$("#lgcsCode").find("option:selected").val();
                formData.lgcsName=$("#lgcsCode").find("option:selected").text();
