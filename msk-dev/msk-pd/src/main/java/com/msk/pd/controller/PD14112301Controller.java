...
import com.msk.pd.logic.PD14112301Logic;
...
public class PD14112301Controller extends BaseController {
    ...
    public String init(Model model, PD141123Bean bean) {
        if (bean.getClassesCode() == null || bean.getClassesCode() == "") throw new BusinessException("数据异常!请联系管理员");
    ...
    String saveAndEdit(Model mv, PD141123Bean bean) {
        if (bean.getClassesCode() == null || bean.getClassesCode() == "") {
            throw new BusinessException("数据异常!请联系管理员");
        }
        if (!StringUtils.isNotEmpty(bean.getMachiningCode())) {
            //添加操作
            int saveYes = this.pd14112301Logic.saveMacANDMacRef(bean.getClassesCode(), bean.getMachiningName());
