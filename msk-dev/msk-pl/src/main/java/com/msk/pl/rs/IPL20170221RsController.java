 ...
 */
@RestController
public class IPL20170221RsController extends BaseRsController {


    private static Logger logger = LoggerFactory.getLogger(IPL20170221RsController.class);


    @Autowired
    private IPL20170221Logic ipl20170221Logic;

    @RequestMapping(value = "/api/v1/pl/partner_fee_list",
            method = RequestMethod.POST,
            produces = {MediaType.APPLICATION_JSON_VALUE})
    @Validator(validatorClass = "com.msk.pl.validator.IPL20170221Validator")
    public RsResponse<IPL20170221RsResult> partnerFeeList(@RequestBody RsRequest<IPL20170221RsParam> param) {
        RsResponse<IPL20170221RsResult> rs = new RsResponse<IPL20170221RsResult>();
        //1:计费查询 2:进货单统计查询
        param.getParam().setSelectType(1);
        ...
        if(result.getTotalCount() == NumberConst.IntDef.INT_ZERO){
            rs.setMessage("合伙人进货单计费查询无数据！");
            rs.setStatus(BusinessConst.RsStatus.FAIL);
        }else{
            rs.setMessage("合伙人进货单计费查询成功！");
