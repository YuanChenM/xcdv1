...
import com.msk.sl.logic.SL2411010401Logic;
...
public class SL2411010401Controller extends BaseUploadController {
    ...
    public void update(MultipartFile file,SlEpHonor slEpHonor,HttpServletRequest request,HttpServletResponse response) throws IOException {
    ...
    public void delete(SlEpHonor slEpHonor,HttpServletRequest request,HttpServletResponse response) throws IOException {
        Long slEpId = (Long)request.getSession().getAttribute("jsp_epId2");
        if(null==request.getSession().getAttribute("jsp_epId2")){
            throw new BusinessException("没有获取到账号信息，请重新进入");
        }
