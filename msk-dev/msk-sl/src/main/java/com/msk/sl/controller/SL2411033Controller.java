...
import com.msk.sl.logic.SL24110310Logic;
...
@Controller
@RequestMapping("SL2411033")
public class SL2411033Controller extends BaseController {
...
//    private String url = StringConst.FILE_SERVICE_URL;//文件服务器路径
//
...
//                }
//                String uploadFilePath= BusinessConst.SLPath.SERVICEIMAGEPATH+"/"+BusinessConst.SLPath.SLIMAGEPATH+"/"+sl24110301Bean.getEpId()+"/";
...
//                /**拼接营业执照图片路径*/
//                sl24110301Bean.setLicPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + licFileName + StringConst.DOT + licSuffix);
...
//                /**拼接税务等级证图片路径*/
//                sl24110301Bean.setTaxPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + taxFileName + StringConst.DOT + taxSuffix);
//                sl24110301Bean.setOrgSpeck("代码：" + sl24110301Bean.getOrgNo());
//                /**拼接组织代码证图片路径*/
//                sl24110301Bean.setOrgNoPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + orgFileName + StringConst.DOT + orgSuffix);
...
//                /**拼接银行开户许可证图片路径*/
//                sl24110301Bean.setBalPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + orgFileName + StringConst.DOT + banSuffix);
...
//                /**拼接三证合一图片路径*/
//                sl24110301Bean.setLicTypePath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + thrFileName + StringConst.DOT + thrSuffix);
//                /**拼接食品流通许可证图片路径*/
//                sl24110301Bean.setFdlPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + epfooFileName + StringConst.DOT + epfooSuffix);
//                /**拼接代理及分销授权书图片路径*/
//                sl24110301Bean.setFdlPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + epageFileName + StringConst.DOT + epageSuffix);
//                /**拼接OEM委托授权图片路径*/
//                sl24110301Bean.setFdlPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + sl24110301Bean.getEpId() + "/" + epoemFileName + StringConst.DOT + epoemSuffix);
//            }
//        }
...
//                    String uploadFilePath=BusinessConst.SLPath.SERVICEIMAGEPATH+"/"+BusinessConst.SLPath.SLIMAGEPATH+"/"+listEpCert.get(i).getEpId()+"/";
//                    String fileName=BusinessConst.SLPath.CERT + certId;
//                    SLUploadFile slUploadFile = new SLUploadFile();
...
//            slCode = "1101010001";
//            mav.addAttribute("slCode", slCode);
//            list = sl241103Logic.selectSlEcTeam();
...
//        // 查询品牌信息 品牌证书 包装 品牌荣誉
//        if (isDebug) {
//            List<SL24110308Bean> list0308 = sl241103Logic.queryList0308();
//            List<SL2411030801Bean> list030801 = sl241103Logic.queryInfoList();
...
//                    }
//                    List<SL2411030801Bean> list030801 = sl241103Logic.findList("findList030801", param);
//                    for(SL2411030801Bean sl2411030801Bean : list030801){
//                        String uploadFilePath=BusinessConst.SLPath.SERVICEIMAGEPATH+"/"+BusinessConst.SLPath.SLIMAGEPATH+"/"+epId+"/";
//                        String fileName=BusinessConst.SLPath.BRANDHONOR+sl2411030801Bean.getBrandId();
//                        SLUploadFile slUploadFile = new SLUploadFile();
//                        String suffix=slUploadFile.getSuffixIfExist(uploadFilePath,fileName);
//                        sl2411030801Bean.setBrandHonorPath(url + "/" + BusinessConst.SLPath.SLIMAGEPATH + "/" + epId + "/" + fileName + StringConst.DOT + suffix);
//                    }
//                    mv.addAttribute("list0308", list0308);
//                    mv.addAttribute("list030801", list030801);
//                }
...
//    }
    @Autowired
    private SL241103Logic sl241103Logic;
    private String url = ConfigManager.getFtpHttpUrl();//文件服务器路径
    ...
    public String init(@RequestParam(required = false) String slCode, @RequestParam(required = false) String epId, Model mav) {
        ...
        if (isDebug) {
            ...
            mav.addAttribute("sL0301Bean", sL24110301Bean);
            // 企业专业资质审批调用
            List<SL24110302Bean> listEpCert = this.slEpCert(mav,epId);
            logger.info("企业专业资质审批执行");
            mav.addAttribute("listEpCert", listEpCert);
            // 卖家电商团队调用
            List<SL24110307Bean> list = this.slOrTeamDS(mav, slCode, epId);
     ...
     * @author pxg
     */
    public List<SL24110302Bean> slEpCert(Model mv,String epId) {
        List<SL24110302Bean> listEpCert = new ArrayList<SL24110302Bean>();
        if (isDebug) {
            listEpCert = sl241103Logic.selectSlEpCert();
            String slSelfFlg = "1";
