...
public class SL241103Controller extends BaseController {
    ...
    public List<SlEpDdBean> querySlEpDd(String epId,Model mav) {
        ...
        if (isDebug) {
            slEpDdList = sl241103Logic.noSlEpDdData();
        } else {
            if (!StringUtil.isNullOrEmpty(epId)) {
                BaseParam param = new BaseParam();
                param.setFilter("epId", epId);
    ...
    @RequestMapping(value = "search",
            method = RequestMethod.POST)
    public @ResponseBody PageResult<SlEpAgentAuth> search(@RequestParam(required = false) String slCode,BasePageParam basePageParam) {
        if (isDebug) {
            logger.info("生产商列表");
            return this.sl241103Logic.selectListPe();
        }
    ...
    @RequestMapping(value = "searchOem",
            method = RequestMethod.POST)
    public @ResponseBody PageResult<SlEpOemAuth> oEMTrade(@RequestParam(required = false) String slCode, BasePageParam basePageParam) {
        logger.info("OEM列表");
        if (isDebug) {
            return sl241103Logic.selectListOem();
        }
