...
var SL24110501 = {
    ...
    checkResultChange: function (tag, trId, index) {
        var $value = $(tag).val();
        if ($value == "2") {
            $.alertMessage.info("<center>不正确原因:<input id='noResult'></center>", function () {
    ...
    checkChange: function (tag, trId, index) {
        var $value = $(tag).val();
        if ($value == "3") {
            $.alertMessage.info("<center>不合格原因:<input id='noResult'></center>", function () {
                var $reason = $("#noResult").val();
                $("#" + trId).children('td').eq(index + 1).html($reason);
            ...
            var hiddenReason = $("#noResult" + trId);
            if (!hiddenReason.val()) {
                $("#reasonDiv").html("<input value='' type='hidden' id='noResult" + trId + "'/>");
            } else {
                hiddenReason.val("");
            }
            $("#" + trId).children('td').eq(index + 1).html("");
